...
-- Level 3: Medium grid (~5k cells, 0.05° grid)
-- Each grid cell is approximately 5.5km x 3.5km at UK latitudes

with grid_cells as (
    select
        floor(longitude / 0.05) as grid_x,
        floor(latitude / 0.05) as grid_y,
        concat('G3_', floor(longitude / 0.05)::text, '_', floor(latitude / 0.05)::text) as grid_code
    from {{ ref('stg_crime') }}
...
select
    ...
    g.grid_x,
    g.grid_y,
    -- Create grid cell polygon (0.05° x 0.05°)
    ST_MakeEnvelope(
        g.grid_x * 0.05, 
        g.grid_y * 0.05, 
        (g.grid_x + 1) * 0.05, 
