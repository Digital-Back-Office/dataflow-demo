...
-- Level 4: Fine grid (~10k cells, 0.03° grid)
-- Each grid cell is approximately 3.3km x 2.1km at UK latitudes

with grid_cells as (
    select
        floor(longitude / 0.03) as grid_x,
        floor(latitude / 0.03) as grid_y,
        concat('G4_', floor(longitude / 0.03)::text, '_', floor(latitude / 0.03)::text) as grid_code
    from {{ ref('stg_crime') }}
...
select
    ...
    g.grid_x,
    g.grid_y,
    -- Create grid cell polygon (0.03° x 0.03°)
    ST_MakeEnvelope(
        g.grid_x * 0.03, 
        g.grid_y * 0.03, 
        (g.grid_x + 1) * 0.03, 
