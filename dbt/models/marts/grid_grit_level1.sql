...
-- Level 1: Coarsest grid (~500-1k cells, 0.15° grid)
-- Each grid cell is approximately 16.5km x 10.5km at UK latitudes
-- Perfect for UK-wide zoomed out view

with grid_cells as (
    select
        floor(longitude / 0.15) as grid_x,
        floor(latitude / 0.15) as grid_y,
        concat('G1_', floor(longitude / 0.15)::text, '_', floor(latitude / 0.15)::text) as grid_code
    from {{ ref('stg_crime') }}
...
select
    ...
    g.grid_x,
    g.grid_y,
    -- Create grid cell polygon (0.15° x 0.15°)
    ST_MakeEnvelope(
        g.grid_x * 0.15, 
        g.grid_y * 0.15, 
        (g.grid_x + 1) * 0.15, 
